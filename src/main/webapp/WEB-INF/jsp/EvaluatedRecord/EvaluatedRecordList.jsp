...
<div class="dangqian">
   <div class="row">
        <div class="col-lg-3 col-lg-offset-5 col-md-5 col-md-offset-2 col-xs-6">
            <p><strong>当前位置：查看已参评的信息</strong></p>
        </div>
...
<form action="evaluatedrecord_FindByExpertUnitOrSpecialty" method="post" id="form">
 <input type="hidden" value="${role.exp_id}" name="expert.exp_id" >
        	...
        	<tr>
		        	<th>
		           		<c:choose>
		           			...
		           			同单位参评人员:<input   type="checkbox" name="expert.exp_unit.uni_id" value="${role.exp_unit.uni_id }" class="from-control ">
		           		...
		           		<c:when test="${choose eq 'unit'}">
		           			同单位参评人员:<input  checked="checked" type="checkbox" name="expert.exp_unit.uni_id" value="${role.exp_unit.uni_id }" class="from-control ">
		           			同专业参评人员:<input   type="checkbox" name="expert.exp_specialty.spec_id" value="${role.exp_specialty.spec_id }" class="from-control">
		           		</c:when>
		           		<c:when test="${choose eq 'all'}">
		           			同单位参评人员:<input  checked="checked" type="checkbox" name="expert.exp_unit.uni_id" value="${role.exp_unit.uni_id }" class="from-control ">
		           			同专业参评人员:<input  checked="checked" type="checkbox" name="expert.exp_specialty.spec_id" value="${role.exp_specialty.spec_id }" class="from-control">
		           		</c:when>
		           		<c:otherwise>
		           			同单位参评人员:<input  type="checkbox" name="expert.exp_unit.uni_id" value="${role.exp_unit.uni_id }" class="from-control ">
		           			同专业参评人员:<input  type="checkbox" name="expert.exp_specialty.spec_id" value="${role.exp_specialty.spec_id }" class="from-control">
